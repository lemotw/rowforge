...
`rowforge-core::Manifest` 加兩個欄位。CLI 與 Studio 共用此型別。變更
於 v1 同步發佈。

```rust
struct Manifest {
    // ... 既有欄位 ...
    build: Option<String>,    // 例如 "go build -o bin/handler ./..."
    run:   String,            // 例如 "bin/handler"   (相對於 handler dir)
}
```

語意:

- `build` 為選填。存在時,CLI 與 Studio 在 spawn `run` 前透過 OS shell
  以 `cwd = <handler_dir>` 執行之。
- `run` 必填。同 `cwd` 語意。Studio 為 smoke test 與共用 CLI 的 exec-run
  兩路徑解析並 spawn 此命令。
- 兩個命令以 shell-words 切詞。第一個 token 走 `PATH` 查找。第一個 token
  解析不到 → `UiError::ToolchainMissing`。
- 加 `build` 為向前相容(`Option`、預設 `None`;第 4 部分 §4.6 容忍 reader)。
  加 `run` 為必填屬於破壞性 manifest 變更 — 同一 v1 release 內把所有
  CLI fixture 的 `run` 補上。

驗證路徑:`validate_manifest`(第 5 部分 §5.4)擴充為驗證兩欄位皆可解析,
且每個第一 token 都在 `PATH` 中。PATH 解析失敗為 `ManifestWarning`,
非 error(他機器 `PATH` 不同仍可能跑得起來)。
...
struct HandlerDetail {
    ...
    last_build: Option<BuildRecord>,        // 記憶體;見 §8.4.7
...
struct BuildRecord {
    ...
    exit_code: i32,
    command: String,                       // 跑時的 manifest.build
...
- `HandlerDetail`:**warm**。
- `BuildRecord`、`SmokeTestReport`:**hot** in-memory;不跨 Studio
  重啟持久化(§8.9 Q3/Q4)。
...
- `Building`:子進程執行中;stderr 以 `BuildEvent::StderrLine` 串流
  到 `handler:build:<name>`(§8.5.2)。
- 終止狀態寫入記憶體中的 `BuildRecord`,每 handler 保留到 Studio 重啟。
- 取消:3 秒軟閾值(第 3 部分 §3.5 對 exec-run 為 10 秒;build 廉價可重跑)。
  低於閾值的 force-kill 不需 Part 7 §7.6.3 為 exec-run hard cancel 要求
  的輸入 token 摩擦。
...
2. 記憶體中的 `BuildRecord` / `SmokeTestReport` 丟棄。重啟後 UI 不得
   顯示陳舊的「last build」。
...
§5.2)。兩種獨立 handle 型別,讓型別系統排除交叉取消。
...
ToolchainMissing { cmd: String, expected_for: String },  // 例如 cmd="go" for "build"
...
5. **`BuildRecord` / `SmokeTestReport` 不可跨重啟持久化。** v1
   in-memory;Studio 重啟後 UI 不得顯示陳舊「last build」。
